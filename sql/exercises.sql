...
SELECT *
FROM customer
WHERE active != 1 OR last_name like '%M';
...
SELECT *
FROM category
WHERE category_id > 4 AND name REGEXP '^[CST].*'; -- I could use three LIKE statements but this is shorter and easier

# 3g. Select all columns minus the password column from the staff table for rows that contain a password.
SELECT
    staff_id,first_name,last_name,address_id,picture,email,store_id,active,username,last_update
FROM staff
...
FROM payment
WHERE convert(payment_date, date) IN ('2005/05/25','2005/05/27','2005/05/29');
...
FROM film
WHERE special_features IS NOT NULL AND length < 120 AND rental_duration BETWEEN 5 AND 7
...
SELECT staff.first_name, staff.last_name, address.address, address.address2, address.district, address.postal_code, city.city
FROM staff
